...
class FilterInterviewController {
  async interviewsByCity({ request }) {
    ...
    if (search) interview.where("search_id", search);

    if (active) interview.where("cities.active", active);

    if (city) interview.where("interviews.city_id", city);

    if (user) interview.where("interviews.user_id", user);
  ...
  async interviewsByUser({ request }) {
    ...
    if (active) interview.where("users.active", active);
  ...
  async getFilteredInterview({ request }) {
    ...
    if (begin) interviews.where("interview_date", ">=", begin);

    if (city) interviews.where("city_id", city);

    if (end) interviews.where("interview_date", "<=", end);

    if (search) interviews.where("search_id", search);

    if (user) interviews.where("user_id", user);
